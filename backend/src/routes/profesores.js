...
const HORARIO_REDIS_KEY = 'chronos:profesores';
...
router.post('/cargar-profesores', async (req, res) => {
  ...
  if (!Array.isArray(profesores) || profesores.length === 0) {
    return res.status(400).json({ error: 'El formato del horario no es válido o está vacío' });
  }

  try {
    const client = getRedisClient();
    await client.json.set(HORARIO_REDIS_KEY, '$', profesores);
    console.log('📥 profesores guardados correctamente en Redis');

    res.status(200).json({ message: 'profesores guardadas en Redis correctamente' });
  } catch (err) {
...
router.get('/cargar-profesores', async (req, res) => {
  try {
    ...
    if (!data) {
      return res.status(404).json({ error: 'No se encontraron profesores cargados' });
    }
